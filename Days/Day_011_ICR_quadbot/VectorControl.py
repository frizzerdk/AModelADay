...
def calculate_wheel_vectors(wheel_positions, icr_x, icr_y, target_speed=1.0, wheel_rotation=0.0):
    """
...
class ICRVisualizer:
    def __init__(self, root):
        ...
        # Rotation slider
        ttk.Label(control_frame, text="Wheel Rotation:").pack()
        self.rotation_slider = ttk.Scale(control_frame, from_=-np.pi, to=np.pi,
                                    variable=self.rotation_var, orient=tk.HORIZONTAL,
                                    command=self.update_plot)
        self.rotation_slider.pack(fill=tk.X)
    ...
    def update_plot(self, *args):
        ...
        wheel_rotation = self.rotation_var.get()  # Get rotation value
        
        # Calculate vectors with wheel position rotation
        angles, speeds, vectors, radii, rotated_positions = calculate_wheel_vectors(
            self.wheel_positions, icr_x, icr_y, target_speed, wheel_rotation
        )
        
        # Plot robot frame using rotated positions
        self.ax.plot(rotated_positions[[0,1,3,2,0], 0], 
                    rotated_positions[[0,1,3,2,0], 1], 
                    'k-', label='Robot Frame')
        ...
        for i, (pos, vec, angle, speed, radius, color, label) in enumerate(
                zip(rotated_positions, vectors, angles, speeds, radii, colors, labels)):
            # Draw wheel path
        ...
        title = f'Robot Configuration (Frame Rotation: {np.degrees(wheel_rotation):.1f}°)'
