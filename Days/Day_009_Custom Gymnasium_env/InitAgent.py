...
class SweepAgentLauncher:
    ...
    def load_initial_config(self):
        ...
        # Load sweep config
        sweep_config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sweep_config.yaml")
        if os.path.exists(sweep_config_path):
            ...
            # If sweep_config.yaml doesn't exist, try loading sweep_config_simple.yaml
            simple_sweep_config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sweep_config_simple.yaml")
            if os.path.exists(simple_sweep_config_path):
                with open(simple_sweep_config_path, 'r') as f:
                    sweep_config_content = f.read()
                self.sweep_config_text.delete('1.0', tk.END)
                self.sweep_config_text.insert(tk.END, sweep_config_content)
